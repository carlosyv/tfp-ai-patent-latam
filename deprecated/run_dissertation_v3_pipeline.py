...
================================
Complete TFP-AI pipeline for dissertation-v3.docx
...
OUT_DIR = RESULTS_DIR / 'benchmark_dissertation_v3'
...
def load_ilostat_employment():
    ...
    Gaps within each country are linearly interpolated.
    """
    ...
    full_frame = pd.DataFrame(
        [(c, y) for c in COUNTRIES for y in range(START_YR, END_YR + 1)],
    ...
    for c in COUNTRIES:
        ...
        obs_orig = emp[emp.Country == c].shape[0]
        print(f"    {c}: {obs_orig} original + {len(sub)-obs_orig} interpolated = "
              f"{len(sub)} obs, range {sub.LABOR_ILOSTAT.min()/1e6:.1f}M–"
...
def extract_data_from_csv():
    ...
    # Interpolate missing control values within country
...
def build_merged_dataset(patents, solow, malmquist, wdi):
    ...
    df['AI_Patent_Stock'] = df.groupby('Country')['AI_Patents'].cumsum()
...
def main():
    ...
    solow_df.to_csv(RESULTS_DIR / 'solow_tfp_dissertation_v3.csv', index=False)
    ...
    mq.to_csv(RESULTS_DIR / 'malmquist_dissertation_v3.csv', index=False)
    ...
    merged.to_csv(RESULTS_DIR / 'merged_dissertation_v3.csv', index=False)
    ...
    print(f"\n  Key outputs:")
    print(f"    • merged_dissertation_v3.csv      (full panel dataset)")
