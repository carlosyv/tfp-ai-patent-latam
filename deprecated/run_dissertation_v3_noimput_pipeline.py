...
========================================
Complete TFP-AI pipeline for dissertation-v4.docx — NO IMPUTATION VARIANT

Differences from v3 (imputation-free):
  - ILOSTAT labor: NO interpolation; country-years with missing employment kept as NaN
  - WDI controls: NO interpolation; missing values kept as NaN
  - AI Patents: NO fillna(0); country-years with no WIPO record kept as NaN
    (explicit zeros in the WIPO file are preserved; unobserved country-years are NaN)
  - PWT HC 2024 extrapolation: RETAINED (low-risk, single-step forward projection)
  - PIM capital stock gap-skipping: RETAINED (GFCF coverage complete for all 7 countries)
...
Pipeline Steps:
  ...
  8. H3: Financial development moderation (interaction term)
  9. H4: Digital infrastructure moderation (interaction term)
 10. H5: Panel quantile regression (τ = 0.10, 0.25, 0.50, 0.75, 0.90)
 11. Cross-sectional dependence diagnostics (Pesaran CD test)
 12. Save all outputs (CSV, JSON, LaTeX, TXT summary)
...
Usage:
  python run_dissertation_v3_pipeline.py
"""
...
CONTROLS_PARS = ['LNPGDP_constant2015', 'OPEN_trade', 'LN_HC_index']
...
def load_ilostat_employment():
    ...
    NO interpolation is applied; country-years with no ILOSTAT observation
    remain NaN and will be excluded from TFP computation and regressions.
    """
    ...
    # No interpolation: keep only observed country-years; NaN = excluded
...
def extract_data_from_csv():
    ...
    # No interpolation: missing control values remain NaN and will be excluded
    # from any regression that uses that control variable.
...
def _parse_wipo_pivot(xlsx_path):
    ...
    # No fillna(0): blank cells remain NaN — caller decides treatment
...
def build_merged_dataset(patents, solow, malmquist, wdi):
    ...
    # This avoids treating non-reporting years as confirmed zero-patent years.
    ...
    wdi_keep = [c for c in wdi_r.columns if c in [
        ...
        'FIN_credit_financial', 'GOV_consumption', 'GOV_expense', 'OPEN_trade',
        'URB_urban_pop', 'INF_internet', 'INF_telephone', 'INF_mobile',
    ]]
    df = df.merge(wdi_r[wdi_keep], on=['Country', 'Year'], how='left')

    # Derived variables
    # log1p(NaN) = NaN — propagates correctly; NaN obs dropped in regressions
    ...
    # Cumulative stock: only accumulate over observed (non-NaN) patent values;
    # any NaN in the sequence propagates NaN forward to avoid spurious stock levels
    ...
    # Interaction terms for H3 and H4
    df['AI_x_FIN'] = df['LN_AI_Patents'] * df['FIN_credit_private']
    df['AI_x_INTERNET'] = df['LN_AI_Patents'] * df['INF_internet']
...
def moderation_analysis(df, y_col, x_ai='LN_AI_Patents',
    ...
    H3 predicts β3 > 0 (financial), H4 predicts β3 > 0 (digital)
    """
    if controls is None:
        controls = CONTROLS_PARS.copy()
...
def main():
    ...
    # ── Step 8: H3 — Financial Moderation (Chapter 7) ─────────────────
    print(f"\n{'═'*70}\nSTEP 8: H3 — Financial Development Moderation\n{'═'*70}")
    h3_results = {}
    for y_col, label in [('ln_TFP', 'Solow'), ('TFP_Change', 'Malmquist')]:
        print(f"\n  {label} TFP:")
        mod = moderation_analysis(
            df, y_col,
            moderator_var='FIN_credit_private',
            interaction_var='AI_x_FIN',
            controls=CONTROLS_PARS + ['FIN_credit_private'],
        )
        ...
        for est, r in mod.items():
            if 'error' in r:
                ...
                print(f"    {est}: β(AI)={r['beta_AI']:+.5f}(p={r['p_AI']:.3f}), "
                      f"β(AI×FIN)={r['beta_interaction']:+.6f}"
                      f"(p={r['p_interaction']:.3f}){_stars(r['p_interaction'])}")
    all_results['H3_financial_moderation'] = h3_results

    # ── Step 9: H4 — Digital Infrastructure Moderation (Chapter 7) ────
    print(f"\n{'═'*70}\nSTEP 9: H4 — Digital Infrastructure Moderation\n{'═'*70}")
    h4_results = {}
    for y_col, label in [('ln_TFP', 'Solow'), ('TFP_Change', 'Malmquist')]:
        print(f"\n  {label} TFP:")
        mod = moderation_analysis(
            df, y_col,
            moderator_var='INF_internet',
            interaction_var='AI_x_INTERNET',
            controls=CONTROLS_PARS + ['INF_internet'],
        )
        ...
        for est, r in mod.items():
            if 'error' in r:
                ...
                print(f"    {est}: β(AI)={r['beta_AI']:+.5f}(p={r['p_AI']:.3f}), "
                      f"β(AI×NET)={r['beta_interaction']:+.6f}"
                      f"(p={r['p_interaction']:.3f}){_stars(r['p_interaction'])}")
    ...
    lines = [
        sep,
        f"DISSERTATION v3 RESULTS: AI Patents → TFP, 7 LAC Countries, {START_YR}–{END_YR}",
        sep, '',
        '=' * 110,
    ...
    # H3
    lines += ['', '=' * 110, 'H3: FINANCIAL MODERATION', '=' * 110]
    for label, mod in h3_results.items():
        lines.append(f"\n  {label} TFP:")
        for est, r in mod.items():
            if 'error' not in r:
                lines.append(f"    {est}: β(AI×FIN)={r['beta_interaction']:+.6f} "
                             f"(p={r['p_interaction']:.3f})")

    # H4
    lines += ['', '=' * 110, 'H4: DIGITAL MODERATION', '=' * 110]
    for label, mod in h4_results.items():
        lines.append(f"\n  {label} TFP:")
        for est, r in mod.items():
            if 'error' not in r:
                lines.append(f"    {est}: β(AI×NET)={r['beta_interaction']:+.6f} "
                             f"(p={r['p_interaction']:.3f})")
